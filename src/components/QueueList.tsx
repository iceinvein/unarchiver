				...
				{isInProgress && item.progress && (
					<div className="space-y-2">
						...
						<div className="text-sm text-default-600">
							...
							<p>
								<span className="font-medium">Files extracted:</span>{" "}
								{item.progress.filesExtracted}
							</p>
							{item.progress.totalBytes && (
								<p>
									<span className="font-medium">Progress:</span>{" "}
									{formatBytes(item.progress.bytesWritten)} /{" "}
				...
				{item.status === "completed" && item.stats && (
					<div className="text-sm text-success-600">
						<p>
							...
							{item.stats.files_extracted} files (
		...
		return (
			<div className="text-center py-12 text-default-400">
				...
				<p className="text-sm mt-2">
					Drag and drop archives or use the file picker to get started
				</p>
			</div>
		);
	}
