...
import { listDirectory, getHomeDirectory } from '../lib/api';
...
export default function FileExplorer({
  ...
  const [contextMenu, setContextMenu] = useState<{ x: number; y: number; archivePath: string } | null>(null);
  ...
  useEffect(() => {
    const initializeHome = async () => {
      try {
        ...
        const errorMsg = err instanceof Error ? err.message : 'Failed to get home directory';
        setError(errorMsg);
    ...
    const loadDirectory = async () => {
      ...
      try {
        ...
        const visibleEntries = entries.filter(entry =>
          !entry.name.startsWith('.') && (entry.isDirectory || entry.isArchive)
        );
        ...
        if (errorMsg.includes('Permission denied') || errorMsg.includes('permission')) {
          showError('Permission denied: You don\'t have access to this directory');
  ...
  const toggleFolder = useCallback(async (nodePath: string[]) => {
    ...
    setTree(prevTree => {
      ...
      const updateNodeAtPath = (nodes: TreeNode[], path: string[], depth: number): TreeNode[] => {
        return nodes.map((node, index) => {
          if (depth === path.length - 1 && index === parseInt(path[depth])) {
            ...
            console.log('Toggling node:', node.entry.name, 'from', node.isExpanded, 'to', newNode.isExpanded);
            ...
            if (newNode.isExpanded && node.children.length === 0) {
              loadChildren(node).then(children => {
                ...
                setTree(prevTree => {
                  const updateWithChildren = (nodes: TreeNode[], path: string[], depth: number): TreeNode[] => {
                    return nodes.map((n, i) => {
                      if (depth === path.length - 1 && i === parseInt(path[depth])) {
                        return { ...n, children, isLoading: false };
                      } else if (depth < path.length - 1 && i === parseInt(path[depth])) {
                        return { ...n, children: updateWithChildren(n.children, path, depth + 1) };
                      }
                      return n;
                    });
                  };
            ...
            return newNode;
          } else if (depth < path.length - 1 && index === parseInt(path[depth])) {
  ...
  const handleItemClick = useCallback((node: TreeNode, nodePath: string[]) => {
    console.log('Item clicked:', node.entry.name, 'isDirectory:', node.entry.isDirectory, 'isArchive:', node.entry.isArchive);
    if (node.entry.isArchive) {
      // Single-click selection for archives
      onArchiveSelect(node.entry.path === selectedArchive ? null : node.entry.path);
  ...
  const handleItemDoubleClick = useCallback((node: TreeNode) => {
  ...
  const flattenTree = useCallback((nodes: TreeNode[], path: string[] = []): Array<{ node: TreeNode; path: string[] }> => {
  ...
  }, [tree, focusedIndex, flattenTree, toggleFolder, handleItemDoubleClick, selectedArchive, onArchiveSelect]);
  ...
  const renderTreeNode = (node: TreeNode, depth: number, nodePath: string[], flatIndex: number) => {
    const isSelected = node.entry.isArchive && node.entry.path === selectedArchive;
      ...
      <div key={node.entry.path}>
        ...
        >
          ...
          {node.entry.isArchive ? (
            <FileArchive className={`w-4 h-4 flex-shrink-0 ${isSelected ? 'text-primary' : 'text-warning'}`} />
          ) : node.entry.isDirectory ? (
            node.isExpanded ? (
          ...
          {/* File name */}
          <span className={`text-sm truncate ${isSelected ? 'font-medium text-primary' : 'text-default-700'}`}>
            {node.entry.name}
          </span>
        ...
        {node.isExpanded && node.children.length > 0 && (
          <div>
            {node.children.map((child, childIndex) => {
              ...
              return renderTreeNode(child, depth + 1, childPath, childFlatIndex);
            })}
          </div>
        )}
      </div>
