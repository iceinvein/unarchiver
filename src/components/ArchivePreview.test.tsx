...
describe('ArchivePreview', () => {
  ...
  it('should render empty state when no archive is selected', () => {
    ...
    expect(screen.getByText('Select an archive to preview its contents')).toBeInTheDocument();
  });
  ...
  it('should show loading state while probing archive', async () => {
    ...
    render(<ArchivePreview archivePath="/test/archive.zip" onExtract={vi.fn()} />);
  ...
  it('should show password protected indicator for encrypted archives', async () => {
    const mockArchiveInfo = {
      ...
      encrypted: true,
      entry_list: [
        { path: 'secret.txt', is_directory: false, size: 1024 },
      ],
    };

    vi.mocked(api.probeArchive).mockResolvedValue(mockArchiveInfo);

    render(<ArchivePreview archivePath="/test/encrypted.zip" onExtract={vi.fn()} />);
  ...
  it('should display error state when probe fails', async () => {
    vi.mocked(api.probeArchive).mockRejectedValue(new Error('Corrupted archive'));

    render(<ArchivePreview archivePath="/test/corrupted.zip" onExtract={vi.fn()} />);
  ...
  it('should show empty archive state when entry list is empty', async () => {
    ...
    render(<ArchivePreview archivePath="/test/empty.zip" onExtract={vi.fn()} />);
