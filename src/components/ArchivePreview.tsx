...
import type { ArchiveInfo, ArchiveEntry } from '../lib/api';
...
export default function ArchivePreview({ archivePath, onExtract }: ArchivePreviewProps) {
  ...
  useEffect(() => {
    ...
    const loadArchive = async () => {
      ...
      try {
        ...
        if (info.encrypted) {
          showWarning('This archive is password-protected. You will be prompted for a password during extraction.');
        }
      } catch (err) {
        const errorMsg = err instanceof Error ? err.message : 'Failed to load archive';
        setError(errorMsg);
        setArchiveInfo(null);
        setTree([]);
    ...
    loadArchive();
  }, [archivePath, buildTree, retryCount]);

  const toggleFolder = useCallback((nodePath: number[]) => {
    setTree(prevTree => {
      // Helper function to deeply clone and update a node
      const updateNodeAtPath = (nodes: TreeNode[], path: number[], depth: number): TreeNode[] => {
  ...
  const renderTreeNode = (node: TreeNode, depth: number, nodePath: number[]) => {
    ...
    return (
      <div key={node.path}>
        ...
        >
          ...
          {/* File name */}
          <span className="text-sm truncate text-default-700">
            {node.name}
          </span>
    ...
    return (
      <div className="flex flex-wrap gap-2">
        ...
        {archiveInfo.encrypted && (
          <Chip size="sm" variant="flat" color="warning" startContent={<Lock className="w-3 h-3" />}>
            Password Protected
          </Chip>
        )}
        <Chip size="sm" variant="flat" color="default">
  ...
  const renderContent = () => {
    ...
    if (isLoading) {
      return (
        <div className="flex flex-col items-center justify-center h-full">
          <Spinner size="lg" />
          <p className="text-sm text-default-500 mt-4">Loading archive contents...</p>
        </div>
      );
    }
    ...
    return (
      <div 
        aria-label="Archive contents tree"
      >
